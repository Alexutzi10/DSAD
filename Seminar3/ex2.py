...
def getAgeGradeOfStudent(students_dict, student_name):
    try:
        (found_age, found_grade) = students_dict[student_name]
...
if searched_grade == None or searched_age == None:
    ...
    print(f"Student: {searched_student}, Age: {searched_age}, Grade: {searched_grade}")
...
def getStudentsDictAboveValue(stud_dict, threshold):
    new_dict = {}
    for entry in stud_dict.keys():
...
students = create_dict(filename)
print("Refactored students:")
print(students)

nota = 8
filtered_students = {student: values for student, values in students.items() if values[1] > 8}
print("Studentii filtrati:")
